...
$app->post('/api/AmazonRedshift/describeClusterVersions', function ($request, $response, $args) {
    ...
    $body = [];
    if(!empty($post_data['args']['clusterParameterGroupFamily'])) {
        $body['ClusterParameterGroupFamily'] = $post_data['args']['clusterParameterGroupFamily'];
    }
    ...
    try {
        $res = $client->describeClusterVersions($body)->toArray();
